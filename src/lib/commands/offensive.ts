...
import { readFile, writeFile, resolvePath, getNode } from '../vfs';
...
export function cmdHydra(args: string[], state: ShellState): CommandResult {
  ...
  // Extract protocol (usually the last arg like ssh, http-post-form, etc.)
  const protocolArg = args.find(a => ['ssh', 'ftp', 'http-post-form', 'http-get-form', 'mysql'].some(p => a.includes(p)));
  let protocol = 'ssh';
  let path = '';
  
  if (protocolArg) {
    if (protocolArg.includes('http')) {
      protocol = 'http';
      // Parse http-post-form format: "/login:user=^USER^&pass=^PASS^:F=failed"
      const parts = protocolArg.split(':');
  ...
  // Find default port for protocol if not specified with -s
  const portArg = args.find(a => a.startsWith('-s'))?.slice(2);
  const port = portArg ? parseInt(portArg) : (protocol === 'http' ? 80 : protocol === 'ssh' ? 22 : protocol === 'ftp' ? 21 : 3306);

  // Get wordlists (simulated)
  const userFile = args.find(a => a.startsWith('-L'))?.slice(2) || args.find(a => a.startsWith('-l'))?.slice(2);
  const passFile = args.find(a => a.startsWith('-P'))?.slice(2) || args.find(a => a.startsWith('-p'))?.slice(2);

  // Simulate brute force delay
  const users = userFile ? ['admin', 'root', 'user'] : ['admin']; // Simplified
  const passes = passFile ? ['password', '123456', 'admin'] : ['password'];
...
export function cmdGobuster(args: string[], state: ShellState): CommandResult {
  const urlArg = args.find(a => a.startsWith('-u'))?.slice(2);
...
export function cmdSqlmap(args: string[], state: ShellState): CommandResult {
  const urlArg = args.find(a => a.startsWith('-u'))?.slice(2);
  const paramArg = args.find(a => a.startsWith('--data'))?.slice(7) || args.find(a => a.includes('&'))?.split('=')[0]; // Rough parsing
...
export function cmdNc(args: string[], state: ShellState, stdin: string): CommandResult {
  ...
  const portArg = args.find((_, i) => i > 0 && !args[i-1].startsWith('-') && isNaN(parseInt(args[i-1]))); // Rough port finding
...
export function cmdSudo(args: string[], state: ShellState, stdin: string): CommandResult {
  // In a real app, we'd prompt for password. Here we just switch user context temporarily
  const originalUser = state.vfs.user;
  state.vfs.user = 'root';
  
  // Execute the next command as root (simplified: just run the first arg as a command)
  // This is a hack for the simulator. In a real shell, we'd re-parse.
  const cmd = args[0];
  const rest = args.slice(1);
  
  // We can't easily recurse here without refactoring executeCommand, so we'll just mock common sudo uses
  if (cmd === 'bash') {
    state.vfs.user = 'root';
    return { output: 'root@shellstrike:/home/hacker# ', exitCode: 0 }; // Change prompt visually
  }
  
  state.vfs.user = originalUser; // Restore
...
export function cmdFind(args: string[], state: ShellState): CommandResult {
  ...
  const pattern = nameFlagIndex !== -1 ? args[nameFlagIndex + 1] : '*';
  
  // Very simplified recursive search
  let output = '';
  // In a real VFS, we'd traverse the tree. Here we just mock a few results for teaching
...
export function cmdTar(args: string[], state: ShellState): CommandResult {
  return { output: 'tar: option requires an argument -- \'f\'\nTry \'tar --help\' for more information.\n', exitCode: 1 };
}
...
export function cmdCrontab(args: string[], state: ShellState): CommandResult {
  if (args.includes('-l')) {
    return { output: 'no crontab for hacker\n', exitCode: 1 };
  }
  return { output: '', exitCode: 0 };
}
...
export function cmdSsh(args: string[], state: ShellState): CommandResult {
  ...
  return { output: `ssh: connect to host ${target} port 22: Connection timed out\n`, exitCode: 1 };
}

export function cmdFtp(args: string[], state: ShellState): CommandResult {
  const target = args[args.length - 1];
  return { output: `Connected to ${target}.\n220 (vsFTPd 3.0.3)\nName (${target}:hacker): \n`, exitCode: 0 };
        }
